...
export async function behandleSchlaferinnerung(request: Request, db: SupabaseClient) {
  ...
  const erledigt = new Set(
    ((naechte ?? []) as Array<{ user_id: string }>).map((zeile) => zeile.user_id)
  )
...
export default {
  fetch: withSupabase(
    ...
    (request, context) =>
      behandleSchlaferinnerung(request, context.supabaseAdmin as SupabaseClient)
  ),
}
